...
public class SupervisorServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<TuitionReimbursementForm> formsDS = dao.getAllForms("PENDING");
		request.getSession().setAttribute("formsDS", formsDS);
		...
		try {
			for(int i = 0; i < username.length; i++) {
				if(status[i].equals("APPROVED")) {
					dao.approveForm(formID[i], "DS-APPROVED");
				}
				if(status[i].equals("DENIED")) {
					dao.denyForm(formID[i], "DS-DENIED");
				}
			}
